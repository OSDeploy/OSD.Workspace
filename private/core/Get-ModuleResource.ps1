Function Get-ModuleResource {
    ...
    try {
        ...
        $JsonContent = Get-Content @params
        if (Test-PSCore) {
            $script:resourceStringsTable = $JsonContent | ConvertFrom-Json -AsHashtable -ErrorAction "Stop"
        }
        else {
            $script:resourceStringsTable = $JsonContent | ConvertFrom-Json -ErrorAction 'Stop' | ConvertTo-Hashtable
        }
