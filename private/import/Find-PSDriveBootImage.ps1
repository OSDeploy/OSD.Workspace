function Find-PSDriveBootImage {
    ...
    $Error.Clear()
    #Write-Host -ForegroundColor DarkCyan "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Start"
    ...
    Write-Verbose "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Found $($WindowsMediaDrives.Count) PSDrives containing Windows Media"
    $WindowsMediaSources = @()
    ...
    if ($WindowsMediaSources) {
        Write-Verbose "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Returning System.IO.FileSystemInfo object"
        return $WindowsMediaSources
    }
    else {
        Write-Verbose "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] No Windows Media found"
        return $null
    }
    #=================================================
