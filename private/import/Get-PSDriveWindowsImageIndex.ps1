function Get-PSDriveWindowsImageIndex {
    ...
    if ($PSDriveWindowsImageFile) {
        $WindowsMediaImages = $PSDriveWindowsImageFile | ForEach-Object {
            ...
            Write-Host -ForegroundColor DarkCyan "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] WindowsImage: $($_.FullName)"
        ...
        Write-Verbose "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Building results"
