function Step-WinPEAppWirelessConnect {
    ...
    if ($WimSourceType -eq 'WinRE') {
        ...
        if (-not (Test-Path -Path $WirelessConnectExe)) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Adding cache content $CacheWirelessConnect"
        ...
        if (Test-Path $WirelessConnectExe) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Using cache content $WirelessConnectExe"
