function Step-WinPEAppMicrosoftDaRT {
    ...
    if (Test-Path $MicrosoftDartCab) {
        if (-not (Test-Path "$CacheMicrosoftDaRT\Toolsx64.cab")) {
            Write-Host -ForegroundColor DarkGray "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Adding cache content $CacheMicrosoftDaRT"
    ...
    if (Test-Path $MicrosoftDartCab) {
        if ($MediaName -match 'public') {
            Write-Host -ForegroundColor DarkGray "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Not adding for Public BootMedia"
        }
        else {
            Write-Host -ForegroundColor DarkGray "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Using cache content $MicrosoftDartCab"
        ...
        Write-Host -ForegroundColor DarkGray "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Install Microsoft Desktop Optimization Pack to add Microsoft DaRT to BootImage"
    }
    ...
    if (Test-Path "$MicrosoftDartConfig") {
        ...
        Write-Host -ForegroundColor DarkGray "[$((Get-Date).ToString('HH:mm:ss'))][$($MyInvocation.MyCommand.Name)] Install Microsoft Deployment Toolkit to add Microsoft DaRT Config to BootImage"
    }
    #=================================================
